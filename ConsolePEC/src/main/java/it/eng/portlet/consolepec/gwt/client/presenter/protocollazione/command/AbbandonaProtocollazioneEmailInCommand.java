...
public class AbbandonaProtocollazioneEmailInCommand extends AbstractConsolePecCommand<ConfermaSceltaProtocollazionePresenter> {

	public AbbandonaProtocollazioneEmailInCommand(ConfermaSceltaProtocollazionePresenter presenter) {
		super(presenter);
	}
	...
	protected void _execute() {
		...
		getPresenter().getDispatchAsync().execute(getPresenter().getCreaFascicoloDTO().getAction(), new AsyncCallback<CreaFascicoloActionResult>() {
			...
			public void onSuccess(CreaFascicoloActionResult result) {
				...
				if (result.getError()) {
					...
					getPresenter().getPecInPraticheDB().remove(getPresenter().getIdEmailIn());
