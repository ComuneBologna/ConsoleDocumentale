...
/**
 * A file servlet supporting resume of downloads and client-side caching and GZIP of text content. This servlet can also be used for images, client-side caching would become more efficient. This
...
public class FileServlet extends HttpServlet {
	...
	private static final long DEFAULT_EXPIRE_TIME = 30000L; // ..ms = 1 week.
	...
	private void processRequest(HttpServletRequest request, HttpServletResponse response, boolean content) throws IOException {
		...
		while (st.hasMoreTokens()) {
			...
			if (tk.toLowerCase().equals("forcedownload")) {
				if (st.hasMoreTokens() && "y".equalsIgnoreCase(st.nextToken())) {
					forcesaveAs = true;
				}
			} else if (tk.equalsIgnoreCase("filepath")) {
				StringBuilder sb = new StringBuilder();
				while (st.hasMoreTokens())
					sb.append(st.nextToken()).append("/");
				if (sb.length() > 0 && sb.charAt(sb.length() - 1) == '/')
					sb.deleteCharAt(sb.length() - 1);
				requestedFile = sb.toString();
			}
		}
		if (forcesaveAs && isAndroid(request)) {
			logger.debug("Ricevuto forceSaveAs, ma viene ignorato, in quanto non supportato su Android");
			forcesaveAs = false;
		}
		logger.debug("forcedownload: {} filePath: {}", forcesaveAs, requestedFile);

		String basePath = this.tempDir;
		...
		String contentFileName = URLDecoder.decode(fileName, "UTF-8");

		/* TODO PULIRE: contentFileName */

		// if (fileName.lastIndexOf('.') > 0) {
		// contentFileName = fileName.substring(0, fileName.lastIndexOf('.')) + fileName.substring(fileName.lastIndexOf('.')).toUpperCase();
		// } else
		// contentFileName = fileName;
		String eTag = contentFileName + "_" + length + "_" + lastModified;
		...
		// extension upper case, richiesto da client android
		// http://stackoverflow.com/questions/4674737/avoiding-content-type-issues-when-downloading-a-file-via-browser-on-android

		response.setHeader("Content-Disposition", disposition + ";filename=\"" + contentFileName + "\"");
