...
/**
 * A {@link Cell} used to render a radio button. The value of the radio button may be toggled using the ENTER key as well as via mouse click.
 */
public class RadioButtonCell extends AbstractEditableCell<Boolean, Boolean> {
	...
	/**
	 * Constructs a new {@link RadioButtonCell} that does not depend on selection and does not handle selection.
	 */
	...
	public void onBrowserEvent(Context context, Element parent, Boolean value, NativeEvent event, ValueUpdater<Boolean> valueUpdater) {
		...
		if ("change".equals(type) || enterPressed) {
			...
			/**
			 * Check the radio button if the enter key was pressed and the cell handles selection or doesn't depend on selection. If the cell depends on selection but doesn't handle selection, then
			 * ignore the enter key and let the SelectionEventManager determine which keys will trigger a change.
			 */
